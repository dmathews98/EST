...
def main():
    dosfile = 'beta_dos_gauss_k12_0.020.dat'
    bandfile = 'beta-sn-bands.dat.gnu'
    ...
    point_names = ['$\Gamma$', 'X', 'M', '$\Gamma$', 'z', 'N', 'P', '$Z_1$', '$\Gamma$']
    band_points = band_data[::50,0][:len(point_names)]

    f, (ax1, ax2) = plt.subplots(1, 2, sharey=True)
    f.suptitle('Beta Band Structure and Density of States')
    ax1.set_ylim([-2, 25])
    ...
    ax1.vlines(band_points,-20,45,color='grey',linestyle='dotted')
    ax1.hlines(EFermi, 0, 45, color='red', linestyle='solid', label='Fermi Level')
